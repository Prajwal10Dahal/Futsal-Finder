...
function AgentLoginPage() {
  ...
  const handleLogin = (event) => {
    ...
    if (username && password) {
      axios
        .post('http://localhost:5000/users/agent-login', {
          name: username,
          password: password,
        })
        .then((response) => {
  ...
  return (
    <div className="agent-login-container">
      <div className="agent-login-box">
        <h1 className="agent-login-heading">Futsal Agent Login</h1>
        <form onSubmit={handleLogin} className="agent-login-form">
          ...
          <button type="submit" className="agent-login-button">
            Log in
          </button>
        ...
        <h6 className="new-account">
          ...
          <button type="Signup" className="signup">
            <a href="/agentsignup">Sign Up for futsal agent</a>
          </button>
        </h6>
      </div>
    </div>
